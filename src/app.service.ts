...
export class AppService {
  ...
  async sendEmail(options) {
    // sending email to specific user using nylas api
    try {
  ...
  async getEmails(options) {
    // getting emails to specific user using nylas api
    try {
      const nylas = await Nylas.with(options.token);
      const arr = [];
  ...
  async saveDraft(
    options, //email save into draft for specific user using nylas api
  ) {
    const nylas = await Nylas.with(options.token);
