...
export class AppController {
  ...
  async callback(
    ...
    if (tokenExists) {
      ...
      const response = await this.httpService.post(process.env.NGROK_URL, {
        token: tkn,
      });
  ...
  async sendMail(@Req() req: Request, @Res() res: Response) {
    ...
    const response = await this.httpService.post(
      `${process.env.NGROK_URL}/getmessage`,
      msg,
    ); //send confirmation message to other machine
    await firstValueFrom(response);
  }
  ...
  async draftMail(@Req() req: Request, @Res() res: Response) {
    ...
    console.log(msg);
    const response = await this.httpService.post(
      `${process.env.NGROK_URL}/getsubarr`,
      msg,
    ); //send emial list to other machine
    await firstValueFrom(response);
  }
  ...
  async saveDraft(@Req() req: Request, @Res() res: Response) {
    ...
    console.log(msg);
    const response = await this.httpService.post(
      `${process.env.NGROK_URL}/getdraftmsg`,
      msg,
    ); //send confirmation message for save draft to other machine
    await firstValueFrom(response);
  }
}
